...
mod tests {
    ...
    fn valid_sign_with_dkg_threshold_signers() {
        ...
        let binding = generate_jwt_header_and_payload(&payload);
        ...
        let signature_b64 = encode_config(
            format!("{:?}:{:?}", group_sig.r.compress().as_bytes(), group_sig.z),
            URL_SAFE_NO_PAD,
        );

        println!("{}.{}", binding, signature_b64);
